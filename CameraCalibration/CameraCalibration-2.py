...
objp = np.zeros((6*9,3), np.float32)#world object points
objp[:,:2] = np.mgrid[0:9,0:6].T.reshape(-1,2) #(no of rows, no of elements)
...
for fname in images:
    ...
    ret, corners = cv.findChessboardCorners(gray, (9,6), None)#7 rows and 6 columns #internal points only are found #ret is true or false if points can be found in an image
...
img = cv.imread('left12.jpg')#here we are applying the calliberation to this particular image.
h,  w = img.shape[:2]
