...
objp = np.zeros((8*6,3), np.float32)#world object points
objp[:,:2] = np.mgrid[0:6,0:8].T.reshape(-1,2) #(no of rows, no of elements)
...
for fname in images:
    ...
    ret, corners = cv.findChessboardCorners(gray,(6,8), None)#7 rows and 6 columns #internal points only are found #ret is true or false if points can be found in an image
...
img = cv.imread('image-10900.png')#here we are applying the calliberation to this particular image.
print(img.shape)
print(img.shape[:2])
h,  w = img.shape[:2]
